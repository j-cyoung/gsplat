...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("project_gaussians_2d_batch_forward", &project_gaussians_2d_batch_forward_tensor);
    m.def("project_gaussians_2d_batch_backward", &project_gaussians_2d_batch_backward_tensor);
    m.def("project_gaussians_2d_scale_rot_batch_forward", &project_gaussians_2d_scale_rot_batch_forward_tensor);
    m.def("project_gaussians_2d_scale_rot_batch_backward", &project_gaussians_2d_scale_rot_batch_backward_tensor);
    m.def("rasterize_batch_forward_sum", &rasterize_batch_forward_sum_tensor);
    m.def("rasterize_batch_backward_sum", &rasterize_batch_backward_sum_tensor);
    m.def("map_gaussian_to_intersects_batch", &map_gaussian_to_intersects_batch_tensor);
    m.def("get_tile_bin_edges_batch", &get_tile_bin_edges_batch_tensor);
}
